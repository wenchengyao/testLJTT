...
public class City_Add_country extends MySuite {
	...
	public void testAddCountry() throws Exception {
		...
		ig.inPage();
		ig.clickAdd("国家", null);
		ig.fillNode("testAddArchCountry", "countryNum", "100.012345", "30.065432",null);
		ig.confirm();

		lg.assertEquals("操作结果 : ",lg.getRespMessage(), "操作成功");

		Map<String,Object> m = jdbcTemplate.queryForMap("select * from sm_architecture t where t.number_='ADcountryNum';");
		...
		lg.assertEquals("name_ :", m.get("name_").toString(),"testAddArchCountry");
		lg.assertEquals("number_ :", m.get("number_").toString(),"ADcountryNum");
