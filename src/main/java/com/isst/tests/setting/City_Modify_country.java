...
public class City_Modify_country extends MySuite {
	...
	public void testModifyCountry() throws Exception {
		...
		ig.inPage();
		ig.clickEdit("国家", "countryTestCity");
		ig.fillNode("testEditArchCountry", null, "100.012345", "30.065432","1234abcd!@#$%^&*()_+-={}[]:\";'<>?,./'");
		ig.confirm();

		lg.assertEquals("操作结果 : ",lg.getRespMessage(), "操作成功");

		Map<String,Object> m = jdbcTemplate.queryForMap("select * from sm_architecture t where t.number_='ADcountryTestCity';");
		...
		lg.assertEquals("level_ :", m.get("level_").toString(),"1");
		lg.assertEquals("name_ :", m.get("name_").toString(),"testEditArchCountry");
		lg.assertEquals("number_ :", m.get("number_").toString(),"ADcountryTestCity");
		lg.assertEquals("parent_id_ :", m.get("parent_id_").toString(),"0");
